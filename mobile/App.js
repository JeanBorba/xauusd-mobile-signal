...
const DXY_PRIMARY = 'https://query1.finance.yahoo.com/v8/finance/chart/DX-Y.NYB?range=5d&interval=5m&includePrePost=true';
const DXY_FALLBACK = 'https://query1.finance.yahoo.com/v8/finance/chart/DX=F?range=5d&interval=5m&includePrePost=true';
const DXY_REFRESH_MS = 15000;
const DXY_STALE_SEC = 120;
const DXY_MAX_AGE_SEC = 600;

function fetchDxyQuote(url, source) {
  return fetch(url, { headers: { Accept: 'application/json' } })
    ...
    .then(j => {
      ...
      const marketTs = Number(meta?.regularMarketTime) * 1000;
      if (!Number.isFinite(price) || price <= 0) throw new Error('DXY inválido');
      const asOf = Number.isFinite(marketTs) && marketTs > 0 ? marketTs : Date.now();
      return { value: price, source, asOf, ageSec: Math.max(0, (Date.now() - asOf) / 1000) };
    });
}

async function fetchDxyRealtime() {
  try { const q = await fetchDxyQuote(DXY_PRIMARY, 'DXY · Yahoo 5M'); return { ...q, stale: q.ageSec > DXY_STALE_SEC }; } catch (_) {}
  try { const q = await fetchDxyQuote(DXY_FALLBACK, 'DXY Futures · Yahoo 5M'); return { ...q, stale: q.ageSec > DXY_STALE_SEC }; } catch (_) {}
  return null;
}
...
export default function App() {
  ...
  const [dxy, setDxy] = useState({ value: null, source: null, asOf: null, ageSec: null, stale: true });
  const [market, setMarket] = useState({ RSI:null, EMA20:null, ADX:null, FVG:[], historyReady:false, historyRequired:40, historySource:null, historyAsOf:null });

  const updateDxy = useCallback(async () => {
    const q = await fetchDxyRealtime();
    if (q) setDxy(q); else setDxy(prev => ({ ...prev, stale: true }));
  }, []);

  useEffect(() => {
    updateDxy();
    dxyTimerRef.current = setInterval(updateDxy, DXY_REFRESH_MS);
    const t = setInterval(() => {
      setClock(new Date());
      setDxy(prev => prev.asOf ? { ...prev, ageSec: Math.max(0, (Date.now() - prev.asOf) / 1000), stale: (Date.now() - prev.asOf) / 1000 > DXY_STALE_SEC } : prev);
    }, 1000);
    return () => { clearInterval(dxyTimerRef.current); clearInterval(t); };
  }, [updateDxy]);
  ...
  const connect = useCallback(() => {
    ...
    ws.onopen = () => {
      ...
      pingRef.current = setInterval(() => { if (ws.readyState === 1) { pingSentRef.current = Date.now(); ws.send(JSON.stringify({ type:'ping' })); } }, 10000);
    };
    ws.onmessage = e => {
      try {
        ...
        if (m.type === 'market_state') {
          ...
          setMarket({ RSI:Number.isFinite(Number(m.RSI))?Number(m.RSI):null, EMA20:Number.isFinite(Number(m.EMA20))?Number(m.EMA20):null, ADX:Number.isFinite(Number(m.ADX))?Number(m.ADX):null, FVG:Array.isArray(m.FVG)?m.FVG:[], historyReady:!!m.historyReady, historyRequired:Number(m.historyRequired)||40, historySource:m.historySource||null, historyAsOf:m.historyAsOf||null });
        } else if (m.type === 'tick') {
          ...
          setMarket(prev => ({ ...prev, historyReady:!!m.ready, historyRequired:Number(m.required)||prev.historyRequired, historySource:m.source||prev.historySource, historyAsOf:m.asOf||prev.historyAsOf }));
  ...
  useEffect(() => { connect(); return () => { clearTimeout(reconnectRef.current); clearInterval(pingRef.current); if (wsRef.current) try { wsRef.current.close(); } catch (_) {} }; }, [connect]);
  ...
  const dxyUnavailable = dxy.value == null || dxyAge > DXY_MAX_AGE_SEC;
  const dxyStatus = dxyUnavailable ? 'SEM FEED' : dxyAge > DXY_STALE_SEC ? 'DADO ATRASADO' : 'LIVE';
  const dxyText = dxyUnavailable ? '--' : `${dxy.value.toFixed(2)} · ${Math.floor(dxyAge)}s`;
  const dxyColor = dxyUnavailable ? '#ff5252' : dxyAge > DXY_STALE_SEC ? '#f0b429' : '#21d39a';
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
    ...
    <View style={styles.card}><Text style={styles.cardTitle}>HORÁRIOS (UTC)</Text><View style={styles.sessionGrid}><Session name="Sydney" open="22:00" close="07:00" now={clock} /><Session name="Tokyo" open="00:00" close="09:00" now={clock} /><Session name="Londres" open="08:00" close="17:00" now={clock} /><Session name="N. York" open="13:00" close="22:00" now={clock} /></View></View>
...
function parseHHMM(value) { const [h, m] = String(value).split(':').map(Number); return h * 60 + m; }
function isSessionActive(date, open, close) { const now = utcMinutes(date), start = parseHHMM(open), end = parseHHMM(close); if (start > end) return now >= start || now < end; return now >= start && now < end; }
