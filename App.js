...
async function fetchDxyRealtime() {
  try {
    const q = await fetchDxyQuote(DXY_PRIMARY, 'DXY · Yahoo 5M');
    return { ...q, stale: q.ageSec > DXY_STALE_SEC };
  } catch (_) {}

  try {
    const q = await fetchDxyQuote(DXY_FALLBACK, 'DXY Futures · Yahoo 5M');
    return { ...q, stale: q.ageSec > DXY_STALE_SEC };
  } catch (_) {}
...
export default function App() {
  ...
  const updateDxy = useCallback(async () => {
    ...
    if (q) setDxy(q);
    else setDxy(prev => ({ ...prev, stale: true }));
  }, []);
  ...
  const connect = useCallback(() => {
    ...
    ws.onmessage = e => {
      try {
        ...
        } else if (m.type === 'history') setCandles(Array.isArray(m.candles) ? m.candles : []);
  ...
  const ind = useMemo(() => {
    ...
    const ema20 = closes.length >= 20 ? closes.slice(-20).reduce((s, v) => s + v, 0) : null;
    return { candles: closed.length, ema20, price };
  }, [candles, price]);
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
    ...
    <View style={styles.card}><Text style={styles.cardTitle}>HORÁRIOS (UTC)</Text><View style={styles.sessionGrid}>
      <Session name="Sydney" active={true} /><Session name="Tokyo" active={true} /><Session name="Londres" active={false} /><Session name="N. York" active={false} />
    </View></View>
    ...
    <View style={styles.card}><Text style={styles.cardTitle}>INDICADORES · TF 5M</Text><Indicator name="RSI (14)" value="AGUARDANDO HISTÓRICO" /><Indicator name="ADX (14)" value="AGUARDANDO HISTÓRICO" /><Indicator name="EMA 20" value={ind.ema20 == null ? 'AGUARDANDO HISTÓRICO' : ind.ema20.toFixed(2)} /></View>
    ...
    <View style={styles.card}><Text style={styles.cardTitle}>ESTRUTURA · SMART MONEY</Text><Indicator name="FVG" value="SEM DADOS" /><Indicator name="OB" value="SEM DADOS" /><Indicator name="BOS / CHoCH" value="SEM DADOS" /></View>
    <View style={styles.card}><Text style={styles.cardTitle}>MOTOR DE SINAIS</Text><Text style={styles.wait}>AGUARDANDO CONFIRMAÇÃO</Text><Text style={styles.sub}>{ind.candles} candles fechados disponíveis. Sem sinal sem dados suficientes e sem confirmação causal.</Text></View>
